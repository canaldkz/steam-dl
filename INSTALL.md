...
## Проще всего — через GUI

```bash
steam-dl gui
```

Откроется веб-интерфейс в браузере (для доступа с телефона:
`steam-dl gui --host 0.0.0.0`). Там: статус окружения, поиск игр, загрузка
`.zip`/`.lua`, галочка «сухой прогон», лог установки и кнопка Store-страницы.
Всё то же, что в CLI ниже, но крупными кнопками под тач.

## Установка игры через терминал (каждый раз)
